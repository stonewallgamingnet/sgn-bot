...
const themes = require('../themes/mafia');
...
class Mafia {
	...
	createCategory() {
		...
		const category = this.guild.channels.create({
			name: this.getTheme().game,
	...
	async createChannels() {
		let channelNames = ['town', 'dead', 'mafia', 'jail'];
		let category = await this.createCategory();
		const theme = this.getTheme();

		let channels = await Promise.all([
			category.children.create({
				name: theme.rooms.town
			}),
			category.children.create({
				name: theme.rooms.dead
			}),
			category.children.create({
				name: theme.rooms.jail
			}),
			category.children.create({
				name: theme.rooms.mafia
			})
		]);
...
// function Mafia({guild, host, client}) {
// 	this.client = client;
// 	this.guild = guild;
// 	this.host = host;
// 	// this.theme = theme;
...
// Mafia.prototype.setTheme = function(theme) {
// 	this.theme = theme;
// }

// Mafia.prototype.getTheme = function() {
// 	return this.theme;
// }

// Mafia.prototype.setHost = function(host) {
// 	this.host = host;
// }

// Mafia.prototype.setClient = function(client) {
// 	this.client = client;
// }

// Mafia.prototype.setGuild = function(guild) {
// 	this.guild = guild;
// }
...
// 	var category = this.guild.createChannel(this.theme, 'category', basePermissions.concat([{
...
// 		Promise.all([
// 			this.guild.createChannel(theme.rooms.town, 'text', basePermissions.concat([
// 					{
// 						id: this.roles.alive.id,
...
// 					}
// 				])),
// 			this.guild.createChannel(theme.rooms.dead, 'text', basePermissions.concat([{
// 					id: this.roles.dead.id,
// 					allow: ['VIEW_CHANNEL', 'SEND_MESSAGES']
// 				},
// 				{
...
// 					deny: ['VIEW_CHANNEL']
// 				}])),
// 			this.guild.createChannel(theme.rooms.mafia, 'text', basePermissions),
// 			this.guild.createChannel(theme.rooms.jail, 'text', basePermissions)
// 		]).then((channels) => {
...
// 			this.channels.town.send('@everyone, Welcome to ' + theme.game + '!');
...
// Mafia.prototype.createCategory = async function() {
// 	var category = this.guild.createChannel(this.theme, 'category', basePermissions.concat([{
// 			id: this.roles.alive.id,
// 			allow: ['VIEW_CHANNEL']
// 		},
